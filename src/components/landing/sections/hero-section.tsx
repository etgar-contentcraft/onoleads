    ...
    >
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto px-5 md:px-8 py-32 md:py-40">
        <div className="max-w-3xl">
          ...
          {subheading && (
            <p
              className="text-lg md:text-xl lg:text-2xl text-white/70 leading-relaxed mb-10 max-w-2xl opacity-0"
          ...
          >
            {ctaText && (
              <button
                onClick={open}
                className="group inline-flex items-center justify-center gap-3 px-10 py-5 rounded-2xl bg-[#B8D900] text-[#2a2628] font-heading font-bold text-lg transition-all duration-300 hover:bg-[#c8e920] hover:shadow-[0_0_50px_rgba(184,217,0,0.4)] hover:scale-[1.03] active:scale-[0.98]"
              >
                {ctaText}
            ...
            {statValue && (
              <div className={`flex items-center gap-4 ${isRtl ? "border-r-2" : "border-l-2"} border-[#B8D900]/40 ${isRtl ? "pr-8" : "pl-8"}`}>
                ...
                {statLabel && (
                  <span className="text-sm md:text-base text-white/60 max-w-[140px] leading-snug">
                    {statLabel}
                  </span>
                )}
              </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 animate-scroll-hint">
        <div className="flex flex-col items-center gap-2">
          <span className="text-white/30 text-xs font-medium">{isRtl ? "גללו למטה" : "Scroll"}</span>
